            ...
            try:
                with transaction.atomic():
                    ...
                    if all(result['status'] == 'success' for result in results):
                        ...
                        raise Exception('Facial processing errors')  # Trigger rollback
...
class UserMailLoginAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            email = form.cleaned_data['username']  # Adjusted to use 'username' which handles email
...
class FacialRecognitionLoginAPIView(APIView):
    def post(self, request):
        ...
        if image_data:
            ...
            # Assuming the user_id returned by FaceMatch can be used to retrieve a user instance
                user = UserProfile.objects.get(id=result['user_id'])
                login(request, user)
            ...
            return Response({'status': 'Error', 'message': 'Invalid image data'}, status=status.HTTP_400_BAD_REQUEST)
